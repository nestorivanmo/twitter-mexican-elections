...
def set_rules(headers, delete, bearer_token):
    sample_rules = [
        ...
        {
            "value": '("Clara Luz" OR @claraluzflores OR to:claraluzflores OR #ClaraGobernadora OR #VotaClara OR #EquipoPorNuevoLeón) lang:es',
            "tag": "Clara Luz",
        },
        {
            "value": '("Samuel Garcia" OR @samuel_garcias OR to:samuel_garcias OR #LaViejaPolítica OR @colosioriojas) lang:es',
            "tag": "Samuel Garcia",
        },
        {
            "value": '("Adrian de la Garza" OR @AdrianDeLaGarza OR to:AdrianDeLaGarza OR #TodoVaAEstarBien) lang:es',
            "tag": "Adrian de la Garza",
        },
        {
            "value": '("Fernando Larrazabal" OR @FerLarrazabalNL OR to:FerLarrazabalNL OR #LarrazabalGobernador) lang:es',
            "tag": "Fernando Larrazabal",
        },
    ]
...
def get_stream(headers, set, bearer_token):
    ...
    with requests.get(
        ...
        tweet_id = int(os.popen("ls data | wc -l").readlines()[0].replace("\n", ""))
        for response_line in response.iter_lines():
            if response_line:
                tweet_id += 1
                json_response = json.loads(response_line)
                print(json.dumps(json_response, indent=4, sort_keys=True))
                with open("data/%d.json" % tweet_id, "w+") as outfile:
